...
두번째 이유는 추상적인 모나드 구조에 대해서는 설명하지만, 실제 우리가 코딩하는 코딩의 요소들 
사이의 관계에 대해 자세히 풀어 설명해주는 글이 드물다는 것이 두번째 이유이다. 
...
`Int=>C[Int]`타입의 함수 둘울 합성하려면 첫번째 함수를 `Int=>Int` 타입으로 만들거나 
두번째 함수를 `C[Int]=>C[Int]` 타입으로 만들 수 있으면 된다. 굳이 `C[T]`를 만든 이유는
부가정보를 넣거나 부가적인 처리를 하기 위한 것인데, 첫번째 함수를 `Int=>Int` 타입으로 
...
[다음 글](https://github.com/enshahar/BasicFPinScala/blob/master/Intermediate/Monad_Rules.md)에서는 위 세가지 모나드 규칙의 
