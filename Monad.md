...
반환되면 좋다. 이를 다른 식으로 말한다면, _여기서 `mkCFun(IntC)(c) == c`여야 한다_ 는 의미이다.

2. `mkCFun(f)(initC(x))`를 생각해 보자. 이 경우 아무 정보도 추가하지 않은 `C[T]`타입의 값에 
`mkCFun(f)`가 적용되는 것이다. 따라서 가능하면 이때 생기는 결과 `C[U]`에 있는 정보는 
`f(x)`에서 만들어지는 정보와 동일하면 좋다. 이를 다른말로 표현한다면, `mkCFun(f)(initC(x)) == f(x)` 여야 한다는 것이다.

3. 두 함수의 합성을 생각해보자. 함수 `f: T=>C[U]`, `g: U=>C[V]`가 있다면, `mkCFun(g)(mkCFun(f)(x))`는 `f`를 `x`에 적용한
결과 생기는 정보에 다시 `g`를 적용한 것이다. 
